...
2. If languages are empty, notice the language they used (typed, or
   `[lang:xx]` from a voice memo: `en`, `de`, `es`, `ja`, `pt`, `fr`, …).
   Ask in **that** language which one they want to practice, and roughly
   what level. One question, then wait. Compose it in the detected language
   — German gets German, Japanese gets Japanese. Do not assume Portuguese,
   French, English, or a country.
...
6. `tutor_config.py set-locale de` (or `en`, `pt`, `ja`, …) — the native
   language, used for scaffolding and the daily nudge copy.
